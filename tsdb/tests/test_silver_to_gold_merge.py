...
@pytest.fixture()
def secret_scope(spark: SparkSession) -> str:
    if spark.catalog._jcatalog.tableExists(
        ...
        if configs["env"] == "dev":
            scope = "dbs-scope-DDPHSS-CSELS-PD-DEV-TOWERSCOUT"
        else:
            scope = "dbs-scope-DDPHSS-CSELS-PD-PRD-TOWERSCOUT"
        return scope
    else:
        raise Exception(
...
def test_create_gold_table_update_query(
    ...
    databricks_utils: DBUtils,
    secret_scope: str,
) -> None:
    """
    Tests query generated by the create_gold_table_update_query function
    """

    DATABRICKS_INSTANCE = databricks_utils.secrets.get(
        scope=secret_scope, key="DATABRICKS-INSTANCE"
    )
    PERSONAL_ACCESS_TOKEN = databricks_utils.secrets.get(
        scope=secret_scope, key="DB-PERSONAL-ACCESS-TOKEN"
    )
    ...
    ), f"Databricks SQL API returned that the query failed. Response was {response['status']}."
    ...
    assert (
        df.count() == 2
    ), f"Exactly two records should have been added to the gold table but actual number added was {df.count()}."
