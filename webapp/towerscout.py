...
def get_user_id_from_token(token):
    try:
        ...
        user_id = decoded_token.get('oid')  # 'oid' is the claim representing the user ID
        return user_id
...
def get_objects():
    try:
        ...
        results_raw = stInstance.get_bboxesfortiles(tiles, exit_events, id(session), request_id, user_id)
        # abort if signaled
        if exit_events.query(id(session)):
            print(" client aborted request.")
            exit_events.free(id(session))
